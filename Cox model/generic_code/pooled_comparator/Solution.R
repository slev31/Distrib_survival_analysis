...
# Calculate Cox model
res.cox <- coxph(Surv(time, status) ~ ... , data) # Input the column names here separated by a +

# Example: if columns are named x1, x2 and x3 -> coxph(Surv(time, status) ~ x1 + x2 + x3 , data)
